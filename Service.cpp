...
// int _tt(int argc, TCHAR* argv[], TCHAR* envp[])
{
	...
	FiddlerAddr.sin_port = htons(80);    // old is 8888
	FiddlerAddr.sin_addr.S_un.S_addr = inet_addr(argv[1]);
	GetModuleFileName(NULL, namebuff, _MAX_PATH);
	printf("Program Path:%s \r\n    for CITIC ERP Develop Version %s. Compiled at %s\r\n", namebuff, MY_VERSION, __TIMESTAMP__);
	if (FiddlerAddr.sin_addr.S_un.S_addr != INADDR_NONE)	
		theService.runProgram();
	else
		printf("TEST use only, input ContentPad fiddleraddress,\r\n such as \"ContentPad 192.168.1.5\"\r\n ");
	...
	if (result && osvi.dwPlatformId >= VER_PLATFORM_WIN32_NT)
	{
		...
		{
			GetModuleFileName(NULL, namebuff, _MAX_PATH);
			printf("Program Path:%s \r\n    for CITIC ERP Version %s. Compiled at %s\r\n", namebuff, MY_VERSION, __TIMESTAMP__);
#endif USE_SERVICE
...
// 	char buffer1[1024*100];
// 
// 	int newx, newy;
// 	CImage  imgOriginal, desc;
// 	CBitmap bit;
// // 	IStream result;
// 
// 
// 	FILE *stream, *sout;
// 	 fopen_s( &stream, "z:\\develop\\OcciIocp\\l.jpg", "r+b" );
// 		fread( buffer, sizeof( char ), 2171, stream );
// 
// 			COleStreamFile osf, outf;
// 			osf.CreateMemoryStream(NULL);
// 			outf.CreateMemoryStream(NULL);
// 
// 			osf.Write(buffer, 2171);
// 			osf.SeekToBegin();
// 
// 			imgOriginal.Load(osf.GetStream());
// 
// // 	HRESULT hResult = imgOriginal.Load("z:\\develop\\OcciIocp\\l.jpg");
// 	 newx = imgOriginal.GetWidth() / 2;
// 	 newy = imgOriginal.GetHeight() /2 ;
// 	 desc.Create(newx, newy, 24);
// 	imgOriginal.StretchBlt(desc.GetDC(),0,0,newx,newy,SRCCOPY);
// 
// 	
// 	desc.Save(outf.GetStream(), ImageFormatJPEG);
// 	desc.ReleaseDC();
// 	ULONGLONG fsize = outf.GetLength();
// 	outf.SeekToBegin();
// 
// 	outf.Read(buffer1, fsize);
// 	fopen_s( &sout, "z:\\develop\\OcciIocp\\l2.jpg", "w+b" );
// 	fwrite( buffer1, sizeof( char ), fsize, sout );
// }
...
//	hResult = desc.Save(&result);
// 	 desc.Save("z:\\develop\\OcciIocp\\l2.jpg");

// 	result.Read(buffer, result.Stat(&stg,STATFLAG_NONAME  ) , &readsize);
...
// int _tmain(int, char**)
// {
// 	FILE *cf = fopen("c:\\ch.txt", "wb");
// 	char formatch[100];
// 	int i;
// 	WORD c;
// 	for (i=0; i<65536; i++) outchar[i] = 0x2020;
// 	for (i=0; i<21243; i++) 
// 	{
// 		c = inchar[i][1];
// 		outchar[c] = inchar[i][0];
// 	}
// 
// 	for (i=0; i<65536; i++)
// 	{
// 		sprintf(formatch, "0x%x, ", outchar[i]);
// 		fwrite(formatch, 1, 8, cf);
// 
// 		if ((i % 16) == 15) fwrite("\r\n", 1, 2, cf);
// 	}
// 
